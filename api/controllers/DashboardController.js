...
module.exports = {
  index: function(req, res, next) {
    gladys.box.get({ user: req.session.User, view: 'dashboard' }).then(function(boxs) {
      return res.view('dashboard/index', {
        User: req.session.User,
        boxs: boxs,
        pageName: req.__('pagename-homepage'),
        basePath: BASE_PATH
      });
    });
  },
  ...
  parametres: function(req, res, next) {
    res.view('parametres/index', {
      User: req.session.User,
      pageName: req.__('pagename-parametres')
    });
  },
    ...
    gladys.module
      ...
      .then(module => {
        ...
        fs.access(moduleConfigViewPath, fs.constants.F_OK, err => {
          ...
          var configurationFunctionExist =
            gladys.modules[module.slug] &&
            gladys.modules[module.slug].setup &&
            typeof gladys.modules[module.slug].setup === 'function';
          res.view('module/module-config-view', {
            User: req.session.User,
            pageName: req.__('pagename-module'),
            module,
            moduleConfigViewPath,
            moduleConfigViewExist,
            configurationFunctionExist
          });
        });
      })
      .catch(next);
